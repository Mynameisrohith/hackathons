...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (user) {
      // After login, redirect to the role-based router
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md card-glass animate-card-enter">
        <CardHeader className="text-center">
          ...
          <CardTitle className="mt-4 text-2xl">{t("adminSignIn")}</CardTitle>
        </CardHeader>
        <CardContent>
          <Button className="w-full" onClick={() => initiateGoogleSignIn(auth)}>
